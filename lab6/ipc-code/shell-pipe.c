...
int num_pipes(char ** tokens){
    int count = 0;
    for (int i = 0; tokens[i] != NULL; i++){
        if (strcmp(tokens[i], "|") == 0) count++;
    }
    return count;
}
bool checkPipe_multi(char ** tokens){
    ...
    for (int icmd = 1; parsed_tokens[icmd] != NULL; icmd++){
        char ** cmd1 = parsed_tokens[icmd - 1];
        char ** cmd2 = parsed_tokens[icmd];

        if (icmd == 1){
            int ret1 = fork();

            if (ret1 == 0){
                close(pipes[icmd - 1][0]);
                dup2(pipes[icmd - 1][1], STDOUT_FILENO);
                execvp(cmd1[0], cmd1);
                printf("This command did not run: %s\n", cmd1[0]);
            }
            else {
                int ret2 = fork();
                if (ret2 == 0){
                    close(pipes[icmd - 1][1]);
                    dup2(pipes[icmd - 1][0], STDIN_FILENO);
                    if (icmd != cmd_no)
                        dup2(pipes[icmd][1], STDOUT_FILENO);
                    execvp(cmd2[0], cmd2);
                    printf("This command did not run: %s\n", cmd2[0]);
                }
                else{
